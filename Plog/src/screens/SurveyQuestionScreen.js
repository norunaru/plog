...
const SurveyQuestionScreen = ({navigation}) => {
  ...
  const environments = ["도심", "바다", "하천", "공원"];
  ...
  const handleCitySelect = (city) => {
    ...
    setSelectedCounty("");
    // 세종특별자치시를 선택한 경우 시/군/구는 빈칸으로 설정하고 바로 동/읍/면 선택
    if (city === "세종특별자치시") {
      setSelectedCounty("없음"); // 시/군/구 없이 빈칸 처리
      setTownModalVisible(true); // 동/읍/면 모달을 바로 띄움
    } else {
      setSelectedTown("");
    }
  };
  ...
  const getTowns = () => {
    let towns;
    // 세종특별자치시는 시/군/구가 없으므로 selectedCounty 없이 필터링
    if (selectedCity === "세종특별자치시") {
      towns = csvData.filter(item => item.sd_nm === selectedCity).map(item => item.emd_nm);
    } else {
      towns = csvData.filter(item => item.sgg_nm === selectedCounty).map(item => item.emd_nm);
    }
...
const styles = StyleSheet.create({
  ...
  cityIcon: {
    ...
    marginLeft: responsiveWidth(24.5),
  },
  cityText: {
    color: '#9B9BA3',
    fontSize: responsiveFontSize(1.6),
